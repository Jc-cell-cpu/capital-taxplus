...
const PopupWidget = () => {
  ...
  const sendEmail = async (formData) => {
    try {
      ...
      const templateParams = {
        ...
        user_email: formData.email // Add the user's email to the templateParams
      };
      ...
      setIsLoading(false); // Hide loader after sending email
    }
  };
  ...
  useEffect(() => {
    ...
    if (isSubmitSuccessful && isSuccess) {
      timeout = setTimeout(() => {
        // reset();
        // setIsSuccess(false);
      }, 5000); // Reset the form and hide success message after 5 seconds
    }
  ...
  <button
    type="submit"
    className="w-full px-3 py-4 text-white bg-gradient-to-r from-purple-500 to-indigo-600 rounded-md focus:outline-none hover:from-indigo-600 hover:to-purple-700 transition duration-300">
    {isSubmitting ? (
      <svg
...
{isLoading && (
  <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
    <div className="w-12 h-12 border-4 border-gray-200 rounded-full border-t-[2px] border-b-[2px] animate-spin bg-gradient-to-r from-purple-500 to-indigo-600"></div>
    <span className="ml-2 text-white">Sending...</span>
        ...
        <button
  className="mt-6 text-white border border-indigo-600 bg-gradient-to-r from-purple-500 to-indigo-600 px-4 py-2 rounded-md focus:outline-none hover:from-indigo-600 hover:to-purple-700 transition duration-300"
  onClick={() => reset()}>
  Go back
</button>
